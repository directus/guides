...
Next.js is a React framework that enables Static Site Generation (SSG), Server-side Rendering (SSR), Client-side Rendering (CSR), _or all three at the same time_. Learn more [here](https://nextjs.org/docs/basic-features/pages).
...
There are two simple requests in this example worth noting. The first is in `index.js` [here](https://github.com/directus/examples/blob/main/nextjs/pages/index.jsx) and the other in `[id].js` [here](https://github.com/directus/examples/blob/main/nextjs/pages/articles/%5Bid%5D.jsx). However, the Directus API is exhaustive for any database linked.
